...
def run_test_episode(model, env, max_steps=1000):
    ...
    while not (terminated or truncated) and idx < max_steps:
        best_q_action = model(torch.tensor(last_observation, dtype=torch.float).unsqueeze(0).to(device)).detach().max(-1).indices.item()
...
def main_cartpole(testing=False, checkpoint=None, optional_name="linear decay greedy"):
    device = torch.device("cuda")
    if testing:
    ...
    sample_size = 300
    rb = ReplayBuffer(batch_size=sample_size)
    ...
    try:
        ...
        while True:
            ...
            if (not testing) and rb.size >= min_rb_size and steps_since_train >= env_steps_before_traing:
                ...
                loss = train_step(m, rb, target_m, discount_factor)
                ...
                if steps_since_target >= target_model_update:
                    ...
                    target_m.load_state_dict(m.state_dict())
                    steps_since_target = 0
                    torch.save(target_m.state_dict(),f"models/cartpole/{env_steps}.pth")
